...
Issues are tracked as GitHub issues via the `gh` CLI (auto-targets the origin repo, `Wangyanlin1996/llm-wiki-agent`). See `docs/agents/issue-tracker.md`.

### Triage labels

Uses the five canonical triage labels verbatim — `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. See `docs/agents/triage-labels.md`.
