---
title: When AI-Based Agents Are Proactive
type: source
tags:
- intent-recommendation
- proactive-agent
- human-factors
- self-determination
date: 2025
source_file: raw/papers/proactive-ai-implications.md
last_updated: 2026-05-28
authors:
- Christopher Diebel
- Marc Goutier
- Martin Adam
year: 2025
venue: Business & Information Systems Engineering (BISE)
citation_count: 22
doi: 10.1007/s12599-024-00918-y
---
## 概要
这篇 BISE 期刊论文揭示，AI agents 的 proactive（主动式）帮助比 reactive（响应式）帮助导致更高的 competence-based self-esteem（基于能力的自尊感）损失和更低的系统满意度，且 AI 知识水平越高此效应越强。基于 self-determination theory（自我决定理论），研究结果凸显了主动式 AI 辅助的人因风险。

## 核心论点
- 主动式 AI 帮助降低用户基于能力的自尊感
- 更高的 AI 知识放大此效应
- 系统满意度在主动式辅助下下降
- Self-determination theory（自我决定理论）解释了心理机制
- 设计者必须平衡主动性与用户自主性（user autonomy）

## 关键引述
> "Proactive AI help reduces user competence self-esteem" — 关键发现：主动式 AI 帮助降低用户基于能力的自尊感

## 关联
- [[IntentRecommendation]] — 主动式推荐中的人因考量
- [[IntentUnderstanding]] — 理解用户对主动性的反应
- [[IntentCommunicationDesign]] — 传达设计必须平衡主动性与自主性