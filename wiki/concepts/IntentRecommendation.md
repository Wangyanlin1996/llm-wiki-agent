...
# 主动意图推荐（Intent Recommendation）

主动意图推荐（Intent Recommendation）指 AI 智能体基于推断的用户意图主动检测并建议行动，从被动范式（需显式指令）转向主动范式（智能体预判需求）。
...
- **[[PIRABench]]**：首个面向 GUI 主动意图推荐的基准；提出 [[PIRF]] 记忆感知状态追踪（memory-aware state-tracking）框架；含交错意图的复杂轨迹
...
1. **DD**（需求检测，Demand Detection）：[[IntentFlow]] 从流式交互中检测用户需求
2. **MM**（记忆建模，Memory Modeling）：混合记忆（workspace、user、global）用于长期上下文
3. **PAS**（主动智能体系统，Proactive Agent System）：闭环检测-记忆-行动基础设施

## 跨方向关联

主动意图推荐依赖 [[AgentMemory]] 存储过往交互和用户偏好（[[PASK]] 混合记忆、[[PIRF]] 状态追踪）。它也需要 [[IntentUnderstanding]] 在推荐前推断潜在意图（[[IntentRL]] 在研究前澄清意图）。
